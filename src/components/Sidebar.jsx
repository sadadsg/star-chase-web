    ...
    <aside className="sticky top-[68px]">
      <div className="bg-white rounded-2xl border border-[#FECDD3] p-5">
        ...
        <div className="mt-4 pt-3.5 border-t border-[#EDF0F5]">
          <h3 className="text-[13px] font-semibold text-[#8E99A8] uppercase tracking-wider mb-2.5">人物简介</h3>
          <div className="text-[14px] text-[#5A6577] leading-relaxed space-y-2">
          ...
          {bioLines.length > 1 && (
            <button
              ...
              className="mt-2 text-[13px] text-[#6366F1] font-medium hover:text-[#4F46E5] transition-colors"
            >
              {expanded ? '收起' : '展开全部'}
            </button>
          )}
        ...
        {artistInfo.works && (
          <div className="mt-4 pt-3.5 border-t border-[#EDF0F5]">
            <h3 className="text-[13px] font-semibold text-[#8E99A8] uppercase tracking-wider mb-2.5">代表作品</h3>
            <div className="space-y-1.5">
              {artistInfo.works.map((work, i) => (
                <div key={i} className="flex items-center justify-between text-[14px]">
                  <span className="text-[#2D3748] font-medium">{work.title}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-[13px] text-[#B0BEC5]">{work.role}</span>
                    <span className="text-[13px] text-[#B0BEC5]">{work.year}</span>
                  </div>
                </div>
              ))}
            </div>
