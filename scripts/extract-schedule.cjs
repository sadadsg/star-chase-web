...
const TEXT_PROMPT = (post) => `你是行程信息抽取助手。以下是一条艺人工作室微博（发布于 ${post.time || '未知时间'}）。请抽取其中明确的、有日期的行程或活动安排，输出 JSON 数组，每项 {"date","time","city","typeName","title","description"}。
规则：
- date 为 YYYY-MM-DD；「今天/今晚/明天」等相对日期基于发布时间推算
- time 为 HH:MM 格式（如 "18:00"）；正文只提到日期没提具体时刻就省略该字段
...
// 文字路径（无 GLM 降级）：类型命中 + 显式日期才产出；时刻保守提取
function extractFromTextFallback(post) {
  ...
  const time = extractTime(post.text)
